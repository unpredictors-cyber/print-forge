...
export default function AccountPage() {
  ...
  return <main className="mx-auto w-full max-w-5xl px-4 py-10 sm:px-6"><header className="mb-8 flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Customer space</p><h1 className="mt-2 text-2xl font-semibold tracking-tight text-foreground">My Account</h1><p className="mt-1 text-sm text-muted-foreground">Welcome back, {user.email}.</p></div><Button variant="outline" size="sm" onClick={async () => { await signOut(); router.push('/') }}><LogOut className="size-4" />Log out</Button></header><AccountTabs /></main>
}
