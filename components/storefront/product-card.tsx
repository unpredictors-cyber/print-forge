...
export function ProductCard({ product, reviews }: { product: Product; reviews: Review[] }) {
  ...
  return <article className="group animate-fade-up relative flex h-full flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
    ...
    <button aria-label={wishlisted ? 'Remove from wishlist' : 'Add to wishlist'} aria-pressed={wishlisted} onClick={() => { setWishlisted((value) => !value); toast(wishlisted ? 'Removed from wishlist' : 'Added to wishlist', { variant: 'success' }) }} className="absolute right-3 top-3 z-10 rounded-full border border-card/70 bg-card/95 p-2 text-foreground shadow-sm transition-colors hover:bg-accent/10 hover:text-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"><Heart className={`size-4 ${wishlisted ? 'fill-primary text-primary' : 'text-foreground'}`} /></button>
